...
function racerStreaks(
  races: Array<{ position: number; max_pos: number }>,
): { win_streak: number; loss_streak: number; current_win_streak: number; current_loss_streak: number } {
...
): Record<string, { win_streak: number; loss_streak: number; current_win_streak: number; current_loss_streak: number }> {
...
function buildAggMap(
  ...
  for (const row of aggregates) {
    aggMap[row.racer_id] = { total_races: row.total_races, wins: row.wins, losses: row.losses }
  }
  return aggMap
}

export async function computeStats(
  filter: { type: "user"; userId: string } | { type: "org"; orgId: number },
): Promise<StatsResult> {
  const aggregates = (filter.type === "user"
    ? await sql`
        SELECT
          rp.racer_id,
  ...
  const totalResult = (filter.type === "user"
    ? await sql`SELECT COUNT(*)::int AS total_races_run FROM races WHERE user_id = ${filter.userId}`
    : await sql`SELECT COUNT(*)::int AS total_races_run FROM races WHERE org_id = ${filter.orgId}`) as unknown as Array<{
    total_races_run: number
  }>
  ...
  return { animals, luckiest, unluckiest, win_streak_holder, loss_streak_holder, total_races_run }
}
